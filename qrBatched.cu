...
void batchedQR(double *csrValbatched, int *csrRow, int *csrCol, double *bBatched, double *xBatched, int m, int n, int nnzA, int batches)
{
    ...
    // CUDA_CHECK(cudaMemcpyAsync(d_csrColIndA, csrCol, sizeof(int) * nnzA,
    //                            cudaMemcpyHostToDevice, stream));
    // CUDA_CHECK(cudaMemcpyAsync(d_csrRowPtrA, csrRow, sizeof(int) * (m + 1),
                               cudaMemcpyHostToDevice, stream));

    // step 4: symbolic analysis
    CUSOLVER_CHECK(cusolverSpXcsrqrAnalysisBatched(cusolverH, m, n, nnzA, descrA, d_csrRowPtrA,
                                                   d_csrColIndA, info));
    ...
    while (batchSizeMax < batchSize) {
        ...
        CUSOLVER_CHECK(cusolverSpDcsrqrBufferInfoBatched(cusolverH, m, n, nnzA,
    ...
    for (int idx = 0; idx < batchSize; idx += batchSizeMax) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(d_csrValA, &csrValbatched[idx * nnzA],
                                   sizeof(double) * nnzA * cur_batchSize, cudaMemcpyHostToDevice,
                                   stream));
        CUDA_CHECK(cudaMemcpyAsync(d_b, &bBatched[idx * m], sizeof(double) * m * cur_batchSize,
                                   cudaMemcpyHostToDevice, stream));
        // solve part of Aj*xj = bj
        CUSOLVER_CHECK(cusolverSpDcsrqrsvBatched(cusolverH, m, n, nnzA, descrA, d_csrValA,
                                                 d_csrRowPtrA, d_csrColIndA, d_b, d_x,
                                                 cur_batchSize, // WARNING: use current batchSize
                                                 info, buffer_qr));
        // copy part of xj back to host
        CUDA_CHECK(cudaMemcpyAsync(&xBatched[m * idx], d_x, sizeof(double) * m * cur_batchSize,
    ...
    for (int batchId = 0; batchId < batchSize; batchId++) {
        // measure |bj - Aj*xj|
        double *csrValAj = &csrValbatched[batchId * nnzA];
        double *xj = &xBatched[batchId * m];
        double *bj = &bBatched[batchId * m];
        // sup| bj - Aj*xj|
        double sup_res = 0;
        for (int row = 0; row < m; row++) {
            const int start = csrRow[row] - baseA;
            const int end = csrRow[row + 1] - baseA;
            double Ax = 0.0; // Aj(row,:)*xj
            for (int colidx = start; colidx < end; colidx++) {
                const int col = csrCol[colidx] - baseA;
...
int main(int argc, char *argv[]) {
    ...
    double *csrValbatched, *bbat, *xbat;
    int *csrRowA, *csrColA;
    cudaMallocManaged(&csrValbatched, sizeof(double) * nnzA * batchSize);
    cudaMallocManaged(&csrRowA, sizeof(int) * (m + 1));
    cudaMallocManaged(&csrColA, sizeof(int) * n);
    cudaMallocManaged(&bbat, sizeof(double) * m * batchSize);
    cudaMallocManaged(&xbat, sizeof(double) * m * batchSize);
    for (int i = 0; i < batchSize * nnzA; i++)
        csrValbatched[i] = csrValA[i % batchSize];
    for (int i = 0; i < m; i++)
    {
        csrRowA[i] = csrRowPtrA[i];
        csrColA[i] = csrColIndA[i];
    }
    for (int i = 0; i < m * batchSize; i++)
        bbat[i] = b[i % batchSize];
    batchedQR(csrValbatched, csrRowA, csrColA, bbat, xbat, m, n, nnzA, batchSize);
