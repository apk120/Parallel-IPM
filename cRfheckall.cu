...
{
    int m = 50, n = 100, batches = 4096, iter = 10, nnz; // 6590, 12879, 2036312
    ...
    loadVec(ib, "B_random.csv");//Btxt
    loadVec(ic, "C_random.csv");//Ctxt
    loadVec(ix, "x_random.csv");//x_initial
    loadVec(is, "s_random.csv");//s_initial
    loadVec(iy, "y_random.csv");
    ...
    loadMat(csrValA, csrRowA, csrColA, "A_random.csv", &m, &nnz);
    loadMat(csrValAT, csrRowAT, csrColAT, "At_random.csv", &n, &nnz);
    ...
    loadMat(csrValADA, csrRowADA, csrColADA, "AAt_random.csv", &r_aat, &nnzAAt);
